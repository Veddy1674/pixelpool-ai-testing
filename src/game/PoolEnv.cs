...
class PoolEnv : IPoolEnv
{
    public PhysicsBody[] BallsBody { get; } = new PhysicsBody[16];

    public bool BallsStill { get; private set; } = true; // needs to be manually set to false when balls are moved (velocities applied)

    // init
    public PoolEnv(PhysicsBody[]? presetBallsBody = null)
    {
        if (presetBallsBody == null)
            // create bodies
            for (int i = 0; i < BallsBody.Length; i++)
                BallsBody[i] = new PhysicsBody(ballMass, ballRadius, ballPositions[i]);
        else
    ...
    public void TickAll(out int ballsFell, out Ending ending)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            for (int j = i + 1; j < 16; j++)
            {
                if (BallsBody[i].CheckCollision(BallsBody[j]))
                    BallsBody[i].ResolveCollision(BallsBody[j]);
            }
        }
        ...
        for (int i = 0; i < 16; i++)
        {
            BallsBody[i].ResolveWallCollisions(poolTableCollision);
        }
    }
...
internal class SaveState(PhysicsBody[] ballsRef) : ISaveState
{
    public readonly PhysicsBody[] ballsReference = ballsRef;
    private readonly bool[] ballsActive = new bool[16];
    private readonly Vector2[] ballsPosition = new Vector2[16];
    private readonly float[] ballsAngle = new float[16];
    private bool saved;

    public SaveState(PoolEnv env) : this(env.BallsBody) { }
    ...
    public ISaveState Save()
    {
        for (int i = 0; i < 16; i++)
        {
            ...
            ballsAngle[i] = ballsReference[i].Angle;
        }
        saved = true;
        return this;
    }
    ...
    public bool Load()
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            ballsReference[i].Angle = ballsAngle[i];
        }
        return true;
    }

    public static bool CopyTo(SaveState source, SaveState target)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            target.ballsAngle[i] = source.ballsAngle[i];
            target.saved = true;
        }
        return true;
    }
